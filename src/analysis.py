...
granularity = 10
df = transform.low_pass_filter(df, sensor_columns, sampling_frequency=(1000 / granularity), cutoff_frequency=5)
...
## Dataset produced until here is used for the neural networks
df.to_pickle("path_to_temp_freq_dataset.pkl")
...
print("SVM train accurancy ({} times average) : {}".format(n_cv_rep, performance_tr_svm/n_cv_rep))
print("SVM test accurancy ({} times average) : {}".format(n_cv_rep, performance_te_svm/n_cv_rep))
...
df = pd.read_pickle("path_to_temp_freq_dataset.pkl")
